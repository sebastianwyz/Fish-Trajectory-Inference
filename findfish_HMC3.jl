...
import ForwardDiff
# -------------------------------------------------------
# Soft barriers per evitare terra / uscita dal bounding box
# -------------------------------------------------------
...
# Coordinates are assumed to be a NamedTuple (x=..., y=...)
...
# -----------
# p(X_t | X_{t_1}, X_{t-2}), movement model (f)
...
# -----------
# p(Y_t | X_t), observation model for accustic receivers (g)
...
# -----------
# p(Y_t | X_t), observation model depth measurements (g)
...
function log_prob_signal(signal, s::NamedTuple, device::DepthGauge, map_int)
    ...
    dist = Normal(max_depth, 10)   # era 2
    logpdf(dist, signal)
...
function log_posterior(S::Vector{T}, Ydepth, Yaccustic, bathymetry_int) where T <: NamedTuple
    ...
    # --- PRIOR SULLA POSIZIONE (acqua + bounding box)
    #for t in 1:tmax
    #    lp += log_p_environment(S[t], bathymetry_int)
    #end

    # --- MODELLO DI MOVIMENTO p(X_t | X_{t-1})
    for t in 2:tmax
        lp += log_p_moveRW(S[t], S[t-1])
    end
    # prior movement
    # --- OSSERVAZIONI p(Y_t | X_t)
    for (t, signal, device) in Ydepth
        t ≤ tmax || continue          # salta misure oltre la traiettoria
...
"""
Simulate a random walk that does not leave the boundary box
"""
...
function infer_trajectories(Ydepth, Yaccustic, bathymetry_interpolated;
    ...
    # (0,1)  →  [xmin,xmax] tramite affinità
...
"""
    map_estimate(Ydepth, Yaccustic, bathymetry_int; s_init, tmax)

Restituisce una traiettoria `S_map` che massimizza il log‑posterior
(condizione di partenza ottimizzata per l’HMC).
"""
function map_estimate(Ydepth, Yaccustic, bathymetry_int;
                      s_init, tmax)
    ...
    v_init = inverse(mapping, s_init)        # vettore iniziale

    # obiettivo: MINIMIZZARE −logposterior
    obj(v) = -log_posterior(transform(mapping, v),
                            Ydepth, Yaccustic, bathymetry_int)
    grad(v) = ForwardDiff.gradient(obj, v)

    # --- ottimizzazione MAP con LBFGS
    opt  = Optim.Options(iterations = 5_000)
    res = Optim.optimize(obj, v_init, Optim.SimulatedAnnealing(), opt)
    v_map = Optim.minimizer(res)
    return transform(mapping, v_map)
end
...
function simulate_bridge(tmax; A, B, σ = 3.0, α = 0.7, bathymetry_int)
    ...
    for t in 2:tmax
        ...
        for _ in 1:40
            ...
            y_cand = y[t-1] + randn()*σ_eff + α * (B.y - y[t-1]) / τ
            b_cand = get_depth((x=x_cand, y=y_cand), bathymetry_int)
            # Vincolo: acqua e profondità simile
...
# Supponiamo che bathymetry_map sia una matrice 2D di profondità
#heatmap(bathymetry_map, color=:blues, title="Fondale (profondità)")

"""

calibrare: basta ritoccare α e β.

usa n_samples un po’ più alti (p. es. 5000) le prime volte per vedere se l’autocorrelazione sui parametri scende; se rimane alta, aumenta α o β.
