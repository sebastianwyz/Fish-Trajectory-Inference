...
# Accustic signals
receiver1 = Receiver((x=100, y=20), k=50.0, dist=30.0)
receiver2 = Receiver((x=300, y=180), dist=50.0)
...
pt = pigeons(
    ...
    reference         = fish_lp,               # prior identico alla target
    seed              = 1234,                  # seme per la riproducibilità
    n_rounds          = 2,                     # fino a 2^5–1 = 31 scans iniziali
    n_chains          = 300,                    # numero di catene parallele
...
cold_last_v = pt_samples.value[end, 1:400] |> vec
...
logposteriors = [logdensity(fish_lp, pt_samples.value[i, 1:400]) for i in 1:size(pt_samples.value, 1)]
...
gif(animation, "C:\\Users\\teresa i robert\\Desktop\\TEST\\channel_wide_maze12_morepenalty_mine_sintTEST2.gif", fps = 10)
